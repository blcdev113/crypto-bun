      ...
      <div className="p-4 space-y-6">
        ...
        <div className="bg-[#1E293B] rounded-lg p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <div className="text-3xl font-bold">{formatCurrency(getTotalAssetsValue())}</div>
            </div>
            <div className="text-right">
              <div className="text-sm text-gray-400 mb-1">Today's PNL</div>
              <div className={`text-xl font-semibold ${todayPnl >= 0 ? 'text-[#22C55E]' : 'text-[#EF4444]'}`}>
          ...
          <div className="grid grid-cols-4 gap-4">
            {actions.map(({ icon: Icon, label, color, bgColor, onClick }) => (
              <button
                key={label}
                onClick={onClick}
                className="bg-[#2D3748] hover:bg-[#374151] rounded-lg p-4 transition-colors flex flex-col items-center justify-center group"
              >
                <div className={`w-12 h-12 ${bgColor} bg-opacity-10 rounded-full flex items-center justify-center mb-3 group-hover:bg-opacity-20 transition-colors`}>
        ...
        <div className="bg-[#1E293B] rounded-lg p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="bg-[#22C55E] bg-opacity-10 text-[#22C55E] px-3 py-1 rounded-full text-sm">
              Active
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="bg-[#2D3748] rounded-lg p-4 text-center">
              ...
              <div className="text-lg font-semibold">{formatCurrency(accountBreakdown.exchange)}</div>
              <div className="text-xs text-gray-500 mt-1">
                {((accountBreakdown.exchange / getTotalAssetsValue()) * 100).toFixed(1)}%
              </div>
            </div>
            <div className="bg-[#2D3748] rounded-lg p-4 text-center">
              <div className="text-sm text-gray-400 mb-1">Perpetual</div>
              <div className="text-lg font-semibold">{formatCurrency(accountBreakdown.perpetual)}</div>
              <div className="text-xs text-gray-500 mt-1">
                {((accountBreakdown.perpetual / getTotalAssetsValue()) * 100).toFixed(1)}%
              </div>
            </div>
            <div className="bg-[#2D3748] rounded-lg p-4 text-center">
              <div className="text-sm text-gray-400 mb-1">Trade</div>
              <div className="text-lg font-semibold">{formatCurrency(accountBreakdown.trade)}</div>
              <div className="text-xs text-gray-500 mt-1">
          ...
          <div className="divide-y divide-gray-700">
            {tokenBalances
              ...
              .map(token => {
                ...
                return (
                  <div key={token.symbol} className="p-4 hover:bg-[#2D3748] transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        {logo ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-[#22C55E] flex items-center justify-center text-white mr-4">
